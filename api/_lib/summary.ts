...
export async function generateComprehensiveWeeklyStats(gameweek: number): Promise<any> {
  console.log(`[api/_lib/summary] Generating stats for GW ${gameweek}`);
  ...
  const managerDataPromises = managerIds.map(async (id) => {
    try {
      ...
      console.warn(`[api/_lib/summary] Failed to fetch picks for manager ${id}. Skipping.`, error);
  ...
  for (const manager of standings) {
    ...
    if (activeChip) {
      ...
      chipsUsed.push({ teamName: manager.entry_name, chip: activeChip, emoji: chipEmoji[activeChip] || 'CHIP' });
    }
