...
export function generateButlerAssessment(data: {
  weeklyStats: any;
}): string {
  ...
  // Top performance phrases (backhanded compliments)
  const topPhrases = [
  ...
  const topPhrase = topPhrases[Math.floor(Math.random() * topPhrases.length)];
  const weakPhrase = weakPhrases[Math.floor(Math.random() * weakPhrases.length)];
